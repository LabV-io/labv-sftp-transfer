...
public class ConfigValidator {
    ...
    public static void validate(Config config) {
        ...
        validateFolders(config.getFolders(), config.getIntervalSeconds());
        validateSftp(config.getSftp());
    }
    
    private static void validateInterval(int intervalSeconds) {
        // Global interval: -1 (run-once) or >= 1
        if (!(intervalSeconds == -1 || intervalSeconds >= 1)) {
            throw new IllegalArgumentException("intervalSeconds must be -1 or >= 1.");
        }
    }
    
    private static void validateFolders(List<Config.FolderConfig> folders, int globalInterval) {
        if (folders == null || folders.isEmpty()) {
            throw new IllegalArgumentException("At least one folder must be configured.");
        }
        for (Config.FolderConfig folder : folders) {
            validateSingleFolder(folder, globalInterval);
        }
    }
    
    private static void validateSingleFolder(Config.FolderConfig folder, int globalInterval) {
        ...
        if ("archive".equalsIgnoreCase(folder.getPostAction())
                && (folder.getArchiveDir() == null || folder.getArchiveDir().isEmpty())) {
            throw new IllegalArgumentException("archiveDir must be set for archived folders. Folder: " + folder.getPath());
        }
        
        // Per-folder interval rules:
        // - Optional
        // - If set: must be >= 1 (never -1)
        // - If global == -1 (run-once), per-folder intervals are ignored at runtime,
        //   but still must be valid if present.
        Integer folderIv = folder.getIntervalSeconds();
        if (folderIv != null) {
            if (folderIv < 1) {
                throw new IllegalArgumentException(
                        "intervalSeconds for folder '" + folder.getPath() + "' must be >= 1 (per-folder -1 is not allowed).");
            }
        }
    }
